...
class Banner extends Base_Controller {
	...
	public function index() {
		...
		$this->lang->load ( 'banner', $langCurrent );
		...
		if ($language_abbr != $this->config->item ( 'language_abbr' ))
			...
			$middle = 'backend/';
		$config ['base_url'] = $this->url ( "banner/index" . "/page" );
		$wheres = array ();
		...
		if (isset ( $searchLang ) && $searchLang != '') {
			$wheres ['banners.language'] = $searchLang;
			...
			if(isset($languageFilter) && $languageFilter!=''){
				$wheres ['banners.language'] = $languageFilter;
		...
		$config ['total_rows'] = $this->Model_Banner->getNumItem ( $wheres, $wheresLike);
		//		$config['total_rows'] = 10;
		$config ['per_page'] = $limitPerPage;
		$bannerList = $this->Model_Banner->getList ( 'banners.*,c.name as cat_name', $page, $limitPerPage, $wheres, $wheresLike, $order );
		for($i = 0; $i < count ( $bannerList ); $i ++) {
			if ($this->Model_Banner->_checkAllTranslate ( $bannerList [$i] ['id'] )) {
				$bannerList [$i] ['translated'] = false;
			} else {
				$bannerList [$i] ['translated'] = true;
			}
		
		}
		//get category list
		$this->load->model ( "Model_Category_Banner" );
		$listCategory = $this->Model_Category_Banner->_getList ( 'id,name', 0, 0, $wheresCategory );
		...
		$data ['layout_content'] = $this->load->view ( "templates/" . $this->mtemplate->_template . "/index_banner", $data, true );
	...
	public function add() {
		...
		$listCategory = $this->Model_Category_Banner->_getList ( 'id,name', 0, 0, $where );
		if (isset ( $id ) && $id != 0) {
			//get list language to translate
			$data ['arr_lang'] = array ();
			foreach ( $arrLang as $key => $value ) {
				if (! $this->Model_Banner->_checkTranslate ( $id, $value )) {
					$data ['arr_lang'] [] = $value;
				}
		...
		$data ['layout_content'] = $this->load->view ( "templates/" . $this->mtemplate->_template . "/add_banner", $data, true );
		$this->load->view ( "templates/" . $this->mtemplate->_template . "/index", $data );
	}
	public function save() {
		...
		if ($this->form_validates () == FALSE) {
			...
			redirect ( $this->url ( 'banner/add/' . $idString ) );
		}
		$image = "";
		$data=array();
		if ($fileName != "") {
			...
			$imageThumbWidthXX = $this->config->item ( 'banner_thumb_widthxx' );
			$imageThumbHeightXX = $this->config->item ( 'banner_thumb_heightxx' );
		...
		if ($id == 0) {
			...
			$dbRet = $this->Model_Banner->_save($data);
			...
			$dbRet = $this->Model_Banner->_save($data,0,$id );
			...
			$data['ordering'] = $ordering;
			$data['category'] = $catId;
			$dbRet = $this->Model_Banner->_save($data, $id );
		...
		redirect ( $this->url ( 'banner' ) );
	}
	function delete() {
		$lang = $this->session->userdata ( "language" );
		$language_abbr = $this->session->userdata ( "language_abbr" );
		$this->lang->load ( 'banner', $lang );
		$id = $this->input->post ( 'id' );
		if (empty ( $id )) {
			$lang = $this->uri->segment ( 1 );
			if (isset ( $lang ) && strlen ( $lang ) == 2) {
