...
class Category_Product extends Base_Controller {
	...
	function index()
	{
		...
		if($language_abbr != $this->config->item('language_abbr'))
			...
			$middle = 'backend/';
		$config['base_url'] = $this->url("category_product/index" . "/page");
		$wheresTotal = array();
		$wheresTotal['category_product.language ='] = $langCurrent;
		$config['total_rows'] = $this->Model_Category_Product->getNumItem($wheresTotal);
//		$config['total_rows'] = 10;
		$config['per_page'] = $limitPerPage;
		$wheres = array();
		$wheres['category_product.language ='] = $langCurrent;
		$categoryList = $this->Model_Category_Product->getList('*',$page,$limitPerPage, $wheres);
		...
		$view->assign('layout_content','index_category_product.tpl',true);
		...
		$view->assign('CATEGORY_ORDER',$this->lang->line('CATEGORY_TAB'),true);
		...
		$numCategory = $this->Model_Category_Product->getNumItem(array("language"=>$langCurrent));
		if($numCategory)
		{
			...
			$strRow = array();
			$this->Model_Category_Product->showCategoryItem(0,$langCurrent,$strRow,$k,$str, 
	...
	public function add() 
	{
		...
		if(!empty($id))
		{
			$categoryInfo=$this->Model_Category_Product->getItemById($id);
		...
		if($translateStatus ==1 && $langTranslate!="")
			$whereParent['category_product.language ='] = $langTranslate;
		else
			$whereParent['category_product.language ='] = $langCurrent;
		$listCategory = $this->Model_Category_Product->getList('idCategory,catName',0,0,$whereParent);
		...
		if($language_abbr != $this->config->item('language_abbr'))
			...
			$middle = 'backend/';
		$urlPost = $this->url("category_product/getcategorylist");
		$view->assign('title_page',$this->lang->line('TITLE_PAGE'),true);
		$view->assign('layout_content','add_category_product.tpl',true);
		//Load language key
		$view->assign('TITLE_PAGE',$this->lang->line('TITLE_PAGE'),true);
		$view->assign('CATEGORY_NAME',$this->lang->line('CATEGORY_NAME'),true);
		$view->assign('CATEGORY_STATUS',$this->lang->line('CATEGORY_STATUS'),true);
	...
	public function save()
	{
		...
		if ($this->form_validates() == FALSE)
		{
			...
			redirect( $this->url('category_product/add/'.$idString));
		}
		$image="";
		if($fileName !="")
		{
			...
			$imageThumbHeight, $imageThumbWidthX, $imageThumbHeightX, $fileName)))
			{
				...
				$this->session->set_flashdata(array('message_content' =>$message,'message_type' =>"error"));
				redirect($this->url('category_product/add'));
			}else {
		...
		{
			//get max rid
			$maxRID = $this->Model_Category_Product->getMaxRID();
			...
			$dbRet = $this->db->insert('category_product'); 
		}
		else
		{
			$this->db->set('catName', $name);
			...
			if($parentId!=0)
				$this->db->set('lever', $lever + 1);
			$dbRet = $this->db->update('category_product');  
		...
		redirect($this->url('category_product/'));
	}
	function delete()
	{
		...
		if(is_array($id)) {
			foreach($id as $item) {
				if($this->Model_Category_Product->isExistCategoryChild($item))
				{
			...
			if($this->Model_Category_Product->isExistCategoryChild($id))
			{
				$message = $this->lang->line('EXIST_CATEGORY_CHILD');
				$this->session->set_flashdata(array('message_content' =>$message,'message_type' =>"error"));
				redirect($this->url('category_product/'));
