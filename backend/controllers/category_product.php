...
class Category_Product extends Base_Controller
{
    public function form_validates ()
    {
        ...
        'rules' => 'required|is_unique[category_product.name]'), 
    ...
    function index ()
    {
        ...
        $countCategory = $this->Model_Category_Product->_getNumItem(
        $wheres, $wheresLike);
        ...
        $strRow = array();
        $list = $this->Model_Category_Product->_getList("*", $page, $limitPerPage, 
        $wheres, $wheresLike, $order);
        for ($i = 0; $i < count($list); $i ++) {
            if ($this->Model_Category_Product->_checkAllTranslate(
            $list[$i]['id']))
                $list[$i]['translated'] = false;
            else
                $list[$i]['translated'] = true;
        }
        $totalItem = $this->Model_Category_Product->_getNumItem($wheres, $wheresLike);
        ...
        "templates/" . $this->mtemplate->_template . "/index_category_product", $data, 
        true);
    ...
    public function add ()
    {
        ...
        $listCategory = $this->Model_Category_Product->_getList('*', 0, 0, $wheres);
        if (empty($listCategory))
            $listCategory = $this->Model_Category_Product->_getList('*', 
            0, 0);
        if (isset($id) && $id != 0) {
            //get list language to translate
            $data['arr_lang'] = array();
            foreach ($arrLang as $key => $value) {
                if (! $this->Model_Category_Product->_checkTranslate(
                $id, $value)) {
                    $data['arr_lang'][] = $value;
                }
            }
    ...
    public function save ()
    {
        ...
        if ($id != 0 && $translateStatus != 1) {
            $config = array(
            array('field' => 'name', 'label' => 'Name', 
            'rules' => 'required|edit_unique[category_product.name.' . $id .
             ']'), 
        ...
        if ($id == 0) {
            ...
            $dbRet = $this->Model_Category_Product->_save($data, 0, $id);
        } else {
            $data['name'] = $name;
            $data['parents'] = (int) $parentId;
            $data['status'] = (int) $status;
            $dbRet = $this->Model_Category_Product->_save($data, $id);
        }
        if (! $dbRet) {
    ...
    function delete ()
    {
        ...
        if (empty($id)) {
            ...
            $nodeData = $this->Model_Category_Product->getNodeInfo(
            $id);
            if ($nodeData['parents'] == 0) {
                ...
                'message_type' => "warning"));
                redirect(
                $this->url('category_product/'));
            }
            $result = $this->Model_Category_Product->getChildCategory(
            $id);
            if (! empty($result)) {
                $this->Model_Category_Product->removeNode(
                $id, 'node');
            } else {
                $this->Model_Category_Product->removeNode(
                $id, 'branch');
