...
class Category_Banner extends Base_Controller
{
    public function form_validates ()
    {
        $config = array(
        array('field' => 'name', 'label' => 'Name', 
        'rules' => 'required|is_unique[category_banner.name]'), 
    ...
    function index ()
    {
        ...
        if ($language_abbr != $this->config->item('language_abbr'))
            ...
            $middle = 'backend/';
        $config['base_url'] = $this->url("category_banner/index" . "/page");
        $wheres = array();
        $wheresLike = array();
        ...
        $countCategory = $this->Model_Category_Banner->_getNumItem(
        $wheres, $wheresLike);
        ...
        $strRow = array();
        $list = $this->Model_Category_Banner->_getList("*", $page, $limitPerPage, 
        $wheres, $wheresLike, $order);
        for ($i = 0; $i < count($list); $i ++) {
            if ($this->Model_Category_Banner->_checkAllTranslate(
            $list[$i]['id']))
                $list[$i]['translated'] = false;
            else
                $list[$i]['translated'] = true;
        }
        $totalItem = $this->Model_Category_Banner->_getNumItem($wheres, $wheresLike);
        ...
        "templates/" . $this->mtemplate->_template . "/index_category_banner", $data, 
        true);
    ...
    public function add ()
    {
        ...
        $listCategory = $this->Model_Category_Banner->_getList('*', 0, 0, $wheres);
        if (empty($listCategory))
            $listCategory = $this->Model_Category_Banner->_getList('*', 
            0, 0);
        if (isset($id) && $id != 0) {
            //get list language to translate
            $data['arr_lang'] = array();
            foreach ($arrLang as $key => $value) {
                if (! $this->Model_Category_Banner->_checkTranslate(
                $id, $value)) {
                    $data['arr_lang'][] = $value;
                }
            }
    ...
    public function save ()
    {
        ...
        if ($id != 0 && $translateStatus != 1) {
            $config = array(
            array('field' => 'name', 'label' => 'Name', 
            'rules' => 'required|edit_unique[category_banner.name.' . $id .
             ']'), 
        ...
        if ($id == 0) {
            ...
            $dbRet = $this->Model_Category_Banner->_save($data, 0, $id);
        } else {
            $data['name'] = $name;
            $data['parents'] = $parentId;
            $data['status'] = $status;
            $dbRet = $this->Model_Category_Banner->_save($data, $id);
        }
        if (! $dbRet) {
        ...
        redirect($this->url('category_banner/'));
    }
    function delete ()
    {
        ...
        if (empty($id)) {
            ...
            $nodeData = $this->Model_Category_Banner->getNodeInfo(
            $id);
            if ($nodeData['parents'] == 0) {
                ...
                'message_type' => "warning"));
                redirect(
                $this->url('category_banner/'));
            }
            $result = $this->Model_Category_Banner->getChildCategory($id);
            if (! empty($result)) {
                $this->Model_Category_Banner->removeNode(
                $id, 'node');
            } else {
                $this->Model_Category_Banner->removeNode(
                $id, 'branch');
            ...
            foreach ($id as $item) {
                $result = $this->Model_Category_Banner->getChildCategory(
                $item);
                if (! empty($result)) {
                    $this->Model_Category_Banner->removeNode(
                    $item, 'node');
    ...
    function updateorder ()
    {
        ...
        foreach ($arrayId as $item) {
            ...
            $this->db->update("category_banner");
        }
        $message = $this->lang->line('SAVE_DATA_SUCCESS');
        $this->session->set_flashdata(
        array('message_content' => $message, 'message_type' => "warning"));
