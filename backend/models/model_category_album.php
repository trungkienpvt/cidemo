<?php
class Model_Category_Album extends CI_Model
{
	var $table_name = 'category_album';
...
//    	echo $this->db->last_query();  exit;
	...
	function showCategoryItem($parent_id, $lang, &$strRow, &$k, $str, $config, $baseUrl)
	{
		...
		if (count($result))
		{
			foreach($result as $cate)
			{
				...
				if($numChildMenu)
				{
				  ...
				  <td >".$cate['status']."</td>
				  <td >".$cate['language']."</td>
				  <td><a href='" . $baseUrl . "category_album/add/id/" . $cate['idCategory'] . "'><img src='" . $config['imagepath'] . "images_admin/user_edit.png' alt='' title='' border= '0' /></a></td>
	              <td><a href='" . $baseUrl . "category_album/delete/id/" . $cate['idCategory'] ."' class='ask'><img src='". $config['imagepath'] ."images_admin/trash.png' alt='' title='' border='0' /></a></td>
				 </tr>";
					$str_new = $str."<span class='gi'>|&mdash;</span>";
					$k +=1;	
					
					self::showCategoryItem($cate["idCategory"], $lang, $strRow, $k, $str_new, $config, $baseUrl);
				}
				else
				{
	             ...
	             <td><a href='" . $baseUrl . "category_album/delete/id/" . $cate['idCategory'] ."' class='ask'><img src='". $config['imagepath'] ."images_admin/trash.png' alt = '' title = '' border = '0' /></a></td>
					...
					$k +=1;
				self::showCategoryItem($cate["idCategory"],$lang,$strRow,$k,$str, $config, $baseUrl);
