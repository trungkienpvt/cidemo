...
//    	echo $this->db->last_query();  exit;
	...
	function showCategoryItem($parent_id, $lang, &$strRow, &$k, $str, $config, $arrLang = array(), $langDefault = "english", $baseUrl)
	{
		...
		if (count($result))
		{
			foreach($result as $cate)
			{
				...
				$numChildMenu = self::getNumItem(array('idParent' =>$cate['idMenu']));
				if($numChildMenu)
				{
					...
					.$cate['idMenu']."' /></td>   
				  <td >".$str.$cate['menuName']."</td>
				  <td >".$cate['status']."</td>
				  <td ><img src ='" . ($cate['images']!= NULL?$config['imageUpload'] . "thumb/".$cate['images']:$config['imagePath'] . 'no-image.jpg') . "'/></td>
				  <td><input onkeypress='return submitEnter(event,this)' type='text' name='". $cate['idMenu'] ."' size='6' id='order_" . $cate['idMenu'] . "' value='". $cate['tab'] ."'/>
				  <input type='hidden' name='orderid[]'   value='". $cate['idMenu']."' />
				  <td >".$cate['language']."</td>";
				foreach($arrLang as $key=>$value){
						$rid = $cate['rid'];
						$dataTMP = self::getItemByRID($rid, $value);
						if(empty($dataTMP)) {
							$strRowTMP.="<td><a href='". $baseUrl ."menu/add/id/" . $cate['idMenu']. "/translate/1/lang/". $value ."'>" . $translate . "</a></td>";
						}
						else {
				...
				$strRowTMP.="	
				  <td><a href='". $baseUrl ."menu/add/id/" . $cate['idMenu'] . "'><img src='" . $config['imagePath'] . "images_admin/user_edit.png' alt='' title='' border= '0' /></a></td>
	              <td><a href='". $baseUrl ."menu/delete/id/" . $cate['idMenu'] ."' class='ask'><img src='". $config['imagePath'] ."images_admin/trash.png' alt='' title='' border='0' /></a></td>
				 </tr>";
				$strRow[] = $strRowTMP;
					$str_new = $str."<span class='gi'>|&mdash;</span>";
					$k +=1;	
					self::showCategoryItem($cate["idMenu"], $lang, $strRow, $k, $str_new, $config, $arrLang, $langDefault, $baseUrl);
				}
				else
				{
					
					$strRowTMP ="<tr >
					<td><input type='checkbox' onclick='isChecked(this);' name='id[]' id='c_".$k."' value='".$cate['idMenu']."' /></td>   
				  <td >".$str.$cate['menuName']."</td>
				  <td >".$cate['status']."</td>
				  <td ><img src ='" . ($cate['images']!= NULL?$config['imageUpload'] . "thumb/".$cate['images']:$config['imagePath'] . 'no-image.jpg')."'/></td>
				  <td><input onkeypress='return submitEnter(event,this)' type='text' name='". $cate['idMenu'] ."' size='6' id='order_" . $cate['idMenu'] . "' value='". $cate['tab'] ."'/>
				...
				$strRow[] = $strRowTMP;
					$k +=1;
				self::showCategoryItem($cate["idMenu"],$lang,$strRow,$k,$str, $config, $arrLang, $langDefault, $baseUrl);
	 ...
	 */
	public function isExistMenuChild($idCat)
	{
