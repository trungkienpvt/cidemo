...
{
	global $db,$theme,$u;
	if(!file_exists("themes/".$theme."/templates/$tpl_file"))
		$theme = "default";
	$template = new template_file("themes/".$theme."/".MAIN_PAGE.".htm");
	$strContent = $template->content;
	$strContent =  str_replace("images/","themes/".$theme."/images/",$strContent);
	$strContent =  str_replace("css/","themes/".$theme."/css/",$strContent);
	$strContent =  str_replace("js/","themes/".$theme."/js/",$strContent);
	$template = new template_file("themes/".$theme."/templates/$tpl_file");
	if ($template->error)
	{
		echo ("Page not found.");
		exit;
	...
	$strContent = str_replace("<!--content-->",$template->content,$strContent);
	$strContent = str_replace("../","themes/".$theme."/",$strContent);
	$strContent = str_replace("@ADMINISTRATION_LBL@",ADMINISTRATION_LBL,$strContent);
	if(file_exists("themes/".$theme."/config.php")){
		include("themes/".$theme."/config.php");
	//	$strContent = str_replace("@content_width@",$content_width,$strContent);
	}
	return $strContent;
}
function Back_Links($str)
{
	// Saving back link
	if(empty($_SESSION['BACK_LINKS_NUM'])||!isset($_SESSION['BACK_LINKS_NUM']))
	{
		$_SESSION["BACK_LINKS_NUM"] = 1;
		$_SESSION["BACK_URL"][1] = $_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING'];
	}else{
		$i = $_SESSION["BACK_LINKS_NUM"];
		if($_SESSION["BACK_URL"][$i] != $_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'])
		{
			$_SESSION["BACK_LINKS_NUM"]++;
			$_SESSION["BACK_URL"][$i+1] = $_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING'];
		}
	}
	return str_replace("@backlink@",'goback.php',$str);
}
function loadAjaxTemplate($tpl_file,$path = '')
{
	global $db,$theme,$lang;
	if(!file_exists($path."themes/".$theme."/templates/$tpl_file"))
		$theme = "default";
	$template = new template_file($path."themes/".$theme."/templates/$tpl_file");
	if ($template->error)
	{
		echo ("Page not found.");
		exit;
	}
	return str_replace("../","themes/".$theme."/",$template->content);
}
...
//function convert string vietnamese to not vietnamese
...
//function convert string vietnamese to not vietnamese 2
