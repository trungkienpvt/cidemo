...
<a href="<?php print $PREDATA['base_url']?>category_album/add" class="bt_green"><span class="bt_green_lft"></span><strong><?php print $this->lang->line("ADD NEW ITEM")?></strong><span class="bt_green_r"></span></a> |
...
<table class="table table-striped">
    	...
    	<tr>
			...
			<?php 
	            if(isset($arrOrder['ordering']) && $arrOrder['ordering']=='asc')
	            	echo '<a class="order" href="'.$PREDATA['base_url'] .'category_album/?order_name=ordering&order_value=desc'.'" title="asc" ><i class="icon-chevron-up"></i>' . $this->lang->line("ORDERING") . '</a>';
	            elseif(isset($arrOrder['ordering']) && $arrOrder['ordering']=='desc')
	            	echo '<a class="order" href="'.$PREDATA['base_url'] .'category_album/?order_name=ordering&order_value=asc'.'" title="desc" ><i class="icon-chevron-down"></i>' . $this->lang->line("ORDERING") . '</a>';
	            else 
    ...
    <a href='<?php print $PREDATA['base_url']?>category_album/add/id/<?php print $id?>/translate/1'><?php print $this->lang->line('TRANSLATE')?></a>
    <?php endif?>
    </td>
    <td><a href="<?php print $PREDATA['base_url']?>category_album/add/id/<?php print $id?>"><img src="<?php print $PREDATA['imagePath']?>edit-icon.png"" alt="" title="" border="0" /></a></td>
    <td><a href="<?php print $PREDATA['base_url']?>category_album/delete/id/<?php print $id?>" class="deleted"><img src="<?php print $PREDATA['imagePath']?>delete-icon.png"" alt="" title="" border="0" /></a></td>
    </tr>
    <?php 
    $count++;
    endforeach;?>
     ...
     <input type="hidden" id="action_form" value="<?php print $PREDATA['base_url']?>category_album/updateorder" name="action_form"/> 
 </form>    
