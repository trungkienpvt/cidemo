...
class Model_Banner extends Model_Language {
	...
	function getList($select = "*", $page = 0, $limit = 0, $where = array(), $wheresLike = array(), $order=array()) {
		...
		$this->db->join ( 'category_banner c', "c.id = $this->table_name.category", 'left' );
		if (! empty ( $where )) {
...
//		exit ();
	...
	function searchData($page = 0, $limit = 0, $arraySearch = array(), $wheres = array()) {
		$this->db->select ( '*' );
		$this->db->from ( $this->table_name );
		$this->db->join ( 'category_banner c', "c.idCategory = $this->table_name.category", 'right' );
		if (! empty ( $wheres )) {
	...
	function countSearch($arraySearch = array(), $wheres = array()) {
		$this->db->select ( '*' );
		$this->db->from ( $this->table_name );
		$this->db->join ( 'category_banner c', "c.category = $this->table_name.category", 'right' );
		if (! empty ( $wheres )) {
			foreach ( $wheres as $key => $value ) {
				$this->db->where ( $key, $value );
			}
