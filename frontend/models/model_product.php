...
class Model_Product extends Model_Language {
	...
	function getList($select = "*", $page = 0, $limit = 0, $where = array(), $wheresLike = array(), $order=array()) {
		...
		$this->db->join ( 'category_product c', "c.id = $this->table_name.category", 'left' );
		if (! empty ( $where )) {
...
/**
     * todo: get category info by id
     */
    function getCategoryInfo($catID)
    {
    	$this->load->model('Model_Category_Product');
    	$catData = $this->Model_Category_Product->getItemById($catID);
		return $catData;    	
